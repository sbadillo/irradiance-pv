...
class PVSystem :
    ...
    def __init__(self, times, latitude, longitude, elevation = 0, surface_azimuth = 180, surface_tilt=0):
    ...
    def __repr__(self):
        ...
        attrs = [self.lat, self.lon, self.surface_azimuth, self.surface_tilt, ]
        return "PV System at Lat {} Lon {}. Array azimuth: {} tilt: {}".format(*attrs)
    
           
    def get_solar_pos(self, pressure = 101325, temperature = 12, delta_t = 67.0, atmos_refract = None):
        """        
        ...
        if not isinstance(self.times, pd.DatetimeIndex):
            try:
                time = pd.DatetimeIndex(self.times)
        ...
        print("delta_t:", delta_t, "atmos_refract", atmos_refract)
